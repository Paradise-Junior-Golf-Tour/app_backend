...
module.exports = {
  ...
  async findAll(ctx) { // called by GET /hello 
  ...
  async findUserEvents(ctx) {
    ...
    const event = await strapi.entityService.findOne('api::event.event', parseInt(identifier), {
      populate: ['users'],
      fields: ['*'],
    });
  ...
  async createNew(ctx) {
    const { eventName, eventDescription } = ctx.request.body.data;
    ...
    const slug = eventName.toLowerCase().replaceAll(/[^a-z0-9]+/gi, "-");
    console.log("[Event API] slug", slug)

    await strapi.entityService.create(apiEvent, {
      data: {
        name: eventName,
    ...
    if (event) {
      await strapi.entityService.create('api::tee-time.tee-time', {
        data: {
          Name: eventName,
          event: [event.id],
          publishedAt: new Date(),
  ...
  async register(ctx) {
    ...
    const transactionNew = await strapi.entityService.create('api::transaction.transaction', {
